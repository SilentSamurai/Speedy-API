...
/// {@code com.github.silent.samurai.speedy.serialization.SpeedyToStructure}.
///
...
public class SpeedyToRecord {
    ...
    /// Flattens the entity for a full-replace (PUT) in place over all non-key fields. Like
    /// {@link #toUpdateColumns} it removes key fields and flattens association FK scalars, but a
    /// non-key field that the client *omitted* is not dropped: if it is a resettable column
    /// (updatable, nullable, and not auto-generated) it is set to {@link SpeedyNull} so the write
    /// nulls the column. Required / read-only / auto-generated fields are left to the database
    /// (required fields are enforced present by validation before this runs). Returns the same
    /// instance.
    public SpeedyEntity toReplaceColumns(SpeedyEntity entity) throws SpeedyHttpException {
        EntityMetadata entityMetadata = entity.getMetadata();
        ...
        for (FieldMetadata fieldMetadata : entityMetadata.getAllNonKeyFields()) {
            boolean supplied = entity.has(fieldMetadata)
                    && !entity.get(fieldMetadata).isEmpty()
                    && !entity.get(fieldMetadata).isNull();
            if (supplied) {
                flattenAssociation(entity, fieldMetadata);
            } else if (fieldMetadata.isUpdatable()
                    && fieldMetadata.isNullable()
                    && !fieldMetadata.isAutoGenerated()) {
                // Omitted (or explicitly null) nullable column -> reset to NULL for full replace.
                entity.put(fieldMetadata, SpeedyNull.SPEEDY_NULL);
            } else if (entity.has(fieldMetadata)) {
                // Not resettable (required / read-only / auto-generated): leave it to the DB.
                entity.remove(fieldMetadata);
            }
        }
        return entity;
