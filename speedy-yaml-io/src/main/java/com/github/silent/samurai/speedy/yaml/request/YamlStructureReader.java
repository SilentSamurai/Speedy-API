...
/// YAML {@link StructureReader}: a thin StAX-style cursor over a Jackson
/// {@link JsonParser} produced by a {@link YAMLFactory}. No document tree and no per-node
/// objects — O(1) allocation beyond the {@link SpeedyValue} result. Owns the leaf
/// {@link ValueType} switch (the read mirror of {@code writeLeaf}): it reads raw scalars
/// straight off the parser and constructs {@link SpeedyValue}s directly, with ENUM /
/// ENUM_ORD built inline from field metadata.
///
/// A near-verbatim port of the JSON reader: YAML's streaming backend emits the same token
/// model, so the sole difference is the {@link YAMLFactory} that produces the parser.
public class YamlStructureReader implements StructureReader {

    private static final YAMLFactory FACTORY = new YAMLFactory();

    private final JsonParser parser;

    public YamlStructureReader(JsonParser parser) {
        this.parser = parser;
    }

    /// Opens a streaming YAML reader over the raw request body — the
    /// {@code byte[] -> StructureReader} factory the provider hands to the shared request
    /// parser (a {@link SpeedyRequestReader}). The read-side mirror of
    /// {@code YamlResponseWriter} being handed to the serializer.
    public static YamlStructureReader over(byte[] rawBody) throws SpeedyHttpException {
        try {
            return new YamlStructureReader(FACTORY.createParser(rawBody));
        } catch (IOException e) {
            throw new BadRequestException("Invalid YAML body", e);
        }
    }
    ...
    public String nextKey() throws SpeedyHttpException {
        ...
        if (t != JsonToken.FIELD_NAME) {
            throw new BadRequestException("Invalid YAML body");
        }
        String name;
        try {
