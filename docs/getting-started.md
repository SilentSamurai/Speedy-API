...
> returned by `dataSourcePerReq()`. A new instance is only created when the `DataSource` reference changes — for example
> when switching tenant databases in a multi-tenant setup. Most single-tenant applications will see exactly one
`QueryProcessor` created for the application lifetime.
