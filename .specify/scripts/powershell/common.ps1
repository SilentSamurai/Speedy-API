            ...
            foreach ($presetId in $sortedPresets)
            {
                ...
                if ((Test-Path $manifest) -and $pyCmd)
                {
                    try
                    {
                        ...
                        $pyArgs = if ($pyCmd.Count -gt 1)
                        {
                            $pyCmd[1..($pyCmd.Count - 1)]
                        }
                        else
                        {
                            @()
...
"@ $manifest $TemplateName 2> $pyStderrFile
    ...
    for ($i = $baseIdx - 1; $i -ge 0; $i--) {
        ...
        switch ($strat)
        {
            ...
            'prepend' {
                $content = "$layerContent`n`n$content"
            }
            'append'  {
                $content = "$content`n`n$layerContent"
            }
            'wrap'    {
