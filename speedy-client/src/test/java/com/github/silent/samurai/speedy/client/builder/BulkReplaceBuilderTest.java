...
class BulkReplaceBuilderTest {
    ...
    void executeShouldUsePutMethod() {
        ...
        speedy.replaceMany("User").items(List.of(node("1", "Alice"))).execute();
    ...
    void requestBodyShouldBeAJsonArray() {
        ...
        speedy.replaceMany("User").items(List.of(node("1", "Alice"), node("2", "Bob"))).execute();
    ...
    void executeShouldReturnResult() {
        ...
        SpeedyResult result = speedy.replaceMany("User").items(List.of(node("1", "Alice"))).execute();
    ...
    @Test
    void listOverloadShouldUsePutMethodAndSendAllItems() {
        AtomicReference<String> capturedMethod = new AtomicReference<>();
        AtomicReference<String> capturedBody = new AtomicReference<>();
        Speedy speedy = speedyWith(request -> {
            capturedMethod.set(request.method());
        ...
        speedy.replaceMany("User", List.of(node("1", "Alice"), node("2", "Bob")));
    ...
    void executeShouldThrowWhenItemsNotSet() {
        ...
        assertThrows(IllegalStateException.class, () -> speedy.replaceMany("User").execute());
    }
}
